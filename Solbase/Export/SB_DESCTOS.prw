...
/*/User Function SB_DESCTOS( _cLocal, _cFileName, lJob )
	...
	Else
		...
		_cQry += "       ZZE_XDLINH, ZZE_XTABEL, ZZE_XORIGC "  														+ CHR(13) + CHR(10)
...
//		_cQry += "   AND ZZE.D_E_L_E_T_	<> '*'"															+ CHR(13) + CHR(10)
		_cQry += "WHERE ZZE.D_E_L_E_T_	<> '*'"															+ CHR(13) + CHR(10) // DJALMA BORGES 08/02/2017
		...
		While ! (cAliasQry)->(Eof())
			...
			_dDtDe := CTOD("01/01/2015")
			_dDtAte:= CTOD("31/12/2040")
...
//			_cFile 	+= IIf(Empty(_cUnidade),"00000000",_cUnidade)													//09-DCTITE
			_cFile 	+= _cProdAnt																					//09-DCTITE // DJALMA BORGES 09/02/2017
			_cFile 	+= U_FORMAT(Val((cAliasQry)->ZZE_XTABEL),'ZN',06)												//10-DCTLIS
			_cFile 	+= Substr(_cXRefSb,1,5)																			//11-DCTIPI
//			_cFile 	+= _cProdAnt																					//12-DCTGRC
			_cFile 	+= If(!Empty((cAliasQry)->ZZE_XGRPCL),"00"+(cAliasQry)->ZZE_XGRPCL,Space(8))					//12-DCTGRC // DJALMA BORGES 09/02/2017
			_cFile 	+= "0999"																						//13-DCTPZM
			_cFile 	+= "S"																							//14-DCTDIG
			_cFile 	+= "N "																							//15-DCTDIFICM
			_xVarTmp	:= U_FORMAT((cAliasQry)->ZZE_XPDESC * 10000,'ZN',08)
			_xVarTmp	:= If((cAliasQry)->ZZE_XTPCAL == "A", "-" + SubStr(_xVarTmp,2,7), _xVarTmp)
			_cFile 	+= _xVarTmp				 																		//16-DCTTAXA
			_cFile 	+= U_FORMAT(Val((cAliasQry)->ZZE_XORDEM),'ZN',06)												//17-DCTORDEM
			_cFile 	+= Space(3)																						//18-DCTTPD
			_cFile 	+= Space(1)																						//19-DCTCTB
			_cFile 	+= Space(3)																						//20-DCTTRB
			_cFile 	+= (cAliasQry)->ZZE_XORIGC																		//21-DCTUFO
			_cFile 	+= (cAliasQry)->ZZE_XDESTC																		//22-DCTUFD
