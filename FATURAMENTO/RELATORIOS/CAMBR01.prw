...
User Function CAMBR01(xAlias, cSeq)
	...
	Local aPergs	:= {}
	Local aParam	:= {}
	Local lImpRom := .F. // DJALMA BORGES 17/01/2017
...
//		cQuery += "			INNER JOIN "+ RetSqlName("SUB") + " SUB" + CRLF  
		...
		cQuery += "			INNER JOIN (" + CRLF
		cQuery += "				SELECT C6_NUM, C6_PRODUTO, C6_ITEM, C6_QTDVEN, C6_XROMANE" + CRLF
		cQuery += "				FROM SC6010 SC6" + CRLF
		cQuery += "				WHERE SC6.D_E_L_E_T_ = ' ' AND C6_FILIAL = '" + _cFilial + "'" + CRLF
		cQuery += "				) SC6" + CRLF
		...
		// IMPRIME O ROMANEIO SOMENTE SE TIVER ALGUM ITEM COM C6_XROMANE = .F.
		(_cAlias)->(dbGoTop())
		While (_cAlias)->(!EOF())
			If (_cAlias)->C6_XROMANE == "F"
				lImpRom := .T.
				Exit
			EndIf
			(_cAlias)->(dbSkip())
		...
		(_cAlias)->(dbGotop()) // SÓ IMPRIME SE QTDLIB FOR IGUAL A QTDVEN - DJALMA BORGES 31/12/2016 
		While (_cAlias)->(!EOF()) // E SE NÃO TIVER BLOQUEIO 02/01/2017
...
Static Function ItensPed(oPrint,i, cNumPed)
	...
	While (_cAlias)->(C9_FILIAL+C9_PEDIDO) == xFilial("SC9")+cNumPed .and. !(_cAlias)->(Eof())
	  	...
	  	If (_cAlias)->UB_XSEQ == cSeqImp .or. cSeqImp == "Todas" // DJALMA BORGES 05/01/2017
	  		...
	  		If (_cAlias)->C6_XROMANE == "F" .or. AllTrim(FunName()) == "CAMBC002" // DJALMA BORGES 17/01/2017

	//			SC6->(DbGoTop())
				SC6->(dbSetOrder(1)) // DJALMA BORGES 17/01/2017
				SC6->(Dbseek(xFilial("SC6") + (_cAlias)->C9_PEDIDO + (_cAlias)->C9_ITEM))
			
				// GRAVA FLAG DE IMPRESSÃO DE ROMANEIO - DJALMA BORGES 17/01/2017
				RECLOCK("SC6", .F.)
					SC6->C6_XROMANE := .T.
				SC6->(MSUNLOCK())
...
Static Function fCalcImp(cCliente,cLoja,cTipo,cProduto,cTes,nQtd,nPrc,nValor)
	...
	If IsInCallStack("TMKA271")
		aImp[1] := cProduto
		aImp[2] := cTes
		aImp[3] := MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_ALIQICM")  //Aliquota ICMS
		aImp[4] := MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_VALICM")  //Valor de ICMS
		aImp[5] := MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_VALIPI")  //Valor de IPI
		aImp[6] := MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_ALIQCOF") //Aliquota de calculo do COFINS
		aImp[7] := MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_ALIQPIS") //Aliquota de calculo do PIS
		aImp[8] := MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_ALIQPS2") //Aliquota de calculo do PIS 2
		aImp[9] := MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_ALIQCF2") //Aliquota de calculo do COFINS 2
		aImp[10]:= MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_DESCZF")  //Valor de Desconto da Zona Franca de Manaus
		aImp[11]:= MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_VALPIS")  //Valor do PIS
		aImp[12]:= MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_VALCOF")  //Valor do COFINS
		aImp[13]:= MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_BASEICM") //Valor da Base de ICMS
		aImp[14]:= MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_BASESOL") //Base do ICMS Solidario
		aImp[15]:= MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_ALIQSOL") //Aliquota do ICMS Solidario
		aImp[16]:= MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_VALSOL" ) //Valor Solidário
		aImp[17]:= MaFisRet(VAL((_cAlias)->C9_ITEM),"IT_MARGEM")  //Margem de lucro para calculo da Base do ICMS Sol.
	Else
		aImp[1] := cProduto
		aImp[2] := cTes
