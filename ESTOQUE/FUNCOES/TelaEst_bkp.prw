...
User Function TelaEst()   
	...
	SETKEY(VK_F5, {|| VisualX()   })
	SETKEY(VK_F6, {|| U_TELAESTA(_cEmpPrcFI, cProd)   }) // PASSAGEM DA STRING DE FILIAIS POR PARÂMETRO - DJALMA BORGES 13/12/2016
...
//		dDataFim	:= SZC->ZC_DTFIM 				
	...
	oSayAtalho := TSay():New( 002,016,{||" || F5 = CONSULTA DE RESERVAS || F6 = PREÇO DE VENDA ||"},oDlg1,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,200,008)
...
//		WHERE C9_PEDIDO IS NULL
	...
	Elseif _cOP == "REFERENCIA"
		...
		//  ZZA_TPREF    1=Opcional;2=Similar;3=Montadora;4=Tecnica                                                                                      
