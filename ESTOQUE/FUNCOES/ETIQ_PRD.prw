...
//	Local _cModelo 	:= 'DATAMAX'
	Local _cModelo 	:= 'DMX' // DJALMA BORGES 22/02/2017
	Local _cPorta 	:= 'LPT1' 
	
	// ativar compartilhamento da impressora
	// net use lpt1: \\[nome da maquina]\[nome do compartilhamento da impressora] /persistent:yes
	// net use lpt1 /delete -> para limpar
	
//	MSCBPRINTER(_cModelo,_cPorta,NIL ,NIL,.F.,NIL,NIL,NIL,256000)
//	MSCBPRINTER(_cModelo, _cPorta, NIL, 25, .F., NIL, NIL, NIL, 256000) // DJALMA BORGES 22/02/2017
	MSCBPRINTER(_cModelo, _cPorta, NIL, 152, .F., NIL, NIL, NIL, 256000) // DJALMA BORGES 22/02/2017
	MSCBCHKStatus(.F.)

	If MsgYesNo("A impressora está pronta?")

		MSCBBEGIN(nMV_PAR03,5)
		...
		MSCBSay(_nLin, _nCol, DTOC(dDataBase) + " " + Time(),"N","0","8") // NO LUGAR
		_nLin += 5
