...
//Variaveis criadas para customização - Caio Maior Garcia - GlobalGCS
Private _cXTpFat  := ""
Private _cSerie   := ""
Private _cNfiscal := ""
Private _cCliente := ""
...
//**************************************************************************************************************
//Feito por Caio - GlobalGCS para tratar questão de Roraima
//**************************************************************************************************************
...
If MV_PAR04 == 2
	...
	If _SF2_->F2_EST $ SuperGetMV("ES_UFICM")
...
//****************************Criado por Caio Garcia - GlobalGCS
Private oFontFat    := TFontEx():New(oDanfe,"Times New Roman",05,05,.F.,.T.,.F.)   
PRIVATE oFontAli    := TFontEx():New(oDanfe,"Times New Roman",07,07,.T.,.T.,.F.)// 4
...
//*********************************************************************************************************************|
//TRATAMENTO PARA PEGAR O CLIENTE E A LOJA, PARA UTILIZAR DEPOIS NO TIPO DO FATURAMENTO - Caio Garcia - GlobalGCS      |
...
For nZ := 1 To nLenDet
	...
	// Raphael Araújo 25/10/2016 - Ajuste do valor V.ST por produto, que estava imprimindo incorretamente
	dbSelectArea("SD2")
	dbSetOrder(3)
	MsSeek(xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA+oDet[nX]:_Prod:_cProd:TEXT)
	// ********************************
...
//*******************CAIO*****************************************************************************************|	
	AADD(aAux, AllTrim(TransForm(_nICMRet,TM(_nICMRet,TamSX3("D2_ICMSRET")[1],TamSX3("D2_ICMSRET")[2]))))   //|
//************************************************************************************************************|	
	...
	aadd(aItens,{;
		...
		SubStr(aAux[6], 1, PosQuebrVal(aAux[6])),;//ST
		SubStr(aAux[7], 1, PosQuebrVal(aAux[7])),;//IPI
	...
	aAux[6]  := SubStr(aAux[6], PosQuebrVal(aAux[6]) + 1)//ST
	aAux[7]  := SubStr(aAux[7], PosQuebrVal(aAux[7]) + 1)//IPI

    lPontilhado := .T.	//Alterado por caio
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]) .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6]) .Or. !Empty(aAux[7])
...
If oIdent:_TpNf:Text == "0"
	aTransp[13] := Posicione("CD5",4,xFilial("CD5")+SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA+"0001","CD5_XMARCA")      
	aTransp[14] := Posicione("CD5",4,xFilial("CD5")+SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA+"0001","CD5_XNUMER")                                                                                                         
EndIf
...
//		oDanfe:SayBitmap(042,000,cLogoD,095,096)
		oDanfe:SayBitmap(045,002,cLogoD,085,090)
	Else
//		oDanfe:SayBitmap(042,000,cLogo,095,096)
		oDanfe:SayBitmap(067,005,cLogoD,085,045)
	EndIF
Endif
...
//***********************************************************************************************************************
// Customizado por Caio Garcia - Global GCS - 25/09/2014                                                                |
//                                                                                                                      |
aAux := {{{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}}}                             //|
nY := 0
For nX := 1 To Len(aFaturas)              

	If Len(aFaturas[1]) > 9 //10 - Raphael Global 13/12/2016

		//Criado para aceitar até 26 duplicatas - Caio Garcia - GlobalGcs   //|
		nY++									                        
		aadd(Atail(aAux)[nY],aFaturas[nX][nX])    
		_lFat := .T.                      
...
//Tratamento para verificar se o faturamento do cliente é semanal ou diário
If MV_PAR04 == 2 //Se 2, SF2

	_cXTpFat := Posicione("SA1",1,xFilial("SA1")+_cCliente+_cLoja,"A1_XTPFAT")
...
If _lFat
	...
	For _nx := 1 To 13
		...
		If _nx < 12	                           
			...
			oDanfe:Box(265,_nCol,296,_nCol2)
			_nCol2 += 46.5
			_nCol  += 46.5                     
...
If _lFat            
	...
	// *********************************************************************************************
	// Raphael F. - Global 09/12/2016
	// OBS: TIPO SEMANAL não terá mais impressão das parcelas, apenas valor total no bloco 'FATURA'.
	// *********************************************************************************************
	If _cXtpFat == "S" // Tipo Faturamento SEMANAL
		oDanfe:Say(270,nColuna,SubStr(aFaturas[1][1],1,12),oFontFat:oFont)
		oDanfe:Say(274,nColuna,"A FATURAR",oFontFat:oFont)
		oDanfe:Say(278,nColuna,Alltrim(aTotais[11]),oFontFat:oFont)		
	Endif
	
	If Len(aFaturas) > 0 .and. _cXtpFat <> "S" // Incluído _cXtpFat <> 'S' - Raphael F. 
		For nY := 1 To Len(aFaturas[1])
			If ny < 14
				oDanfe:Say(270,nColuna,aFaturas[1][nY],oFontFat:oFont)
				oDanfe:Say(274,nColuna,aFaturas[2][nY],oFontFat:oFont)
				oDanfe:Say(278,nColuna,aFaturas[3][nY],oFontFat:oFont)
				nColuna:= nColuna+46.5                                
			...
			Else
				...
				oDanfe:Say(285,nColuna,aFaturas[1][nY],oFontFat:oFont)
				oDanfe:Say(290,nColuna,aFaturas[2][nY],oFontFat:oFont)						
				oDanfe:Say(295,nColuna,aFaturas[3][nY],oFontFat:oFont)
				nColuna:= nColuna+46.5   
...
/*	If Len(aFaturas) > 0
		For nY := 1 To Len(aFaturas[1])
			If ny < 14
				...
				If _cXtpFat == "S" .And. !Empty(aFaturas[2][nY])                                        
					oDanfe:Say(274,nColuna,"A FATURAR",oFontFat:oFont)					
				Else
					oDanfe:Say(274,nColuna,aFaturas[2][nY],oFontFat:oFont)
				EndIf	
			...
			Else
				...
				If _cXTpFat == "S" .And. !Empty(aFaturas[2][nY])
					oDanfe:Say(290,nColuna,"A FATURAR",oFontFat:oFont)
				Else
					oDanfe:Say(290,nColuna,aFaturas[2][nY],oFontFat:oFont)						
				EndIf
...
Else
	...
	If _cXTpFat == "S" 
		...
		oDanfe:Say(281,nColuna,"A FATURAR",oFont08:oFont)				
...
/*	If Len(aFaturas) > 0 
		For nY := 1 To 9
			...
			If _cXTpFat == "S" .And. !Empty(aAux[1][nY][2])
				oDanfe:Say(281,nColuna,"A FATURAR",oFont08:oFont)				
			Else
				oDanfe:Say(281,nColuna,aAux[1][nY][2],oFont08:oFont)
			EndIf	
...
EndIf	                                                                                                                                //|
                                                                                                                                        //|
//****************************************************************************************************************************************|
// FIM DA CUSTOMIZAÇÃO DO QUADRO DE FATURAS                                                                                               |
//****************************************************************************************************************************************|
...
For nX :=1 To nLenItens
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][01])
	nY++
	aadd(Atail(_aBase)[nY],NoChar(aItens[nX][02],lConverte))
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][03])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][04])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][05])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][06])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][07])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][08])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][09])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][10])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][11])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][12])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][13])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][14])
	nY++
	aadd(Atail(_aBase)[nY],aItens[nX][15])
	If nY >= 15
		nY := 0
	EndIf
Next nX            
...
aAuxCabec := {;
	...
	"BC.ICMS",;
	"V.ICMS",;
	"V.ST",;//Campo adicionado conforme solicitação do usuário - Caio Garcia - Global Gcs
	"V.IPI",;
	"A.ICMS",;
	"A.IPI";
}
...
//**************************************************************************|
// Adicionado coforme solicitação do cliente - Caio Garcia - GlobalGCS      |
nAuxH += aTamCol[11]                                                      //|
oDanfe:Box(442, nAuxH, 678, nAuxH + aTamCol[12])                          //|
oDanfe:Say(450, nAuxH + 2, "V.ST", oFont08N:oFont)                        //|
//**************************************************************************|
nAuxH += aTamCol[12]                                                         
...
nAuxH += aTamCol[14]
oDanfe:Box(442, nAuxH, 678, nAuxH + aTamCol[15])
...
If _lUf
	...
	For _nx := 1 To Len(aAux[1,14])
	                                           
	    If !Empty(aAux[1,14,_nx])    
	            
		 	aAux[1,14,_nx] := Val(aAux[1,14,_nx])
	...
	fNewIten(aAux,_aBase)	
		    		                   
	_cAliqAnt :=	aAux[1][14][1] 
...
For nY := 1 To nLenItens
	...
	If !Empty(aAux[1][3][nY]) .And. aAux[1][3][nY] <> "-" 
		
		_cAliqAtu := aAux[1][14][nY]
...
//				oDanfe:SayBitmap(000+nLinhavers,000,cLogoD,095,096)
				oDanfe:SayBitmap(000+nLinhavers,000,cLogoD,085,045)
			Else
				oDanfe:SayBitmap(000+nLinhavers,000,cLogo,095,096) 
				oDanfe:SayBitmap(000+nLinhavers,000,cLogo,085,045)
			EndIf
			...
			//**************************************************************************|
			// Adicionado coforme solicitação do cliente - Raphael F. Araújo - Global   |
			nAuxH += aTamCol[11]                                                      //|
			oDanfe:Box(163+nLinhavers, nAuxH, nColLim, nAuxH + aTamCol[12])	          //|
			oDanfe:Say(171+nLinhavers, nAuxH + 2, "V.ST", oFont08N:oFont)           //|
			//**************************************************************************|
			nAuxH += aTamCol[12]
	...
	//		oDanfe:SayBitmap(000,000,cLogoD,095,096)
			oDanfe:SayBitmap(000,000,cLogoD,085,045)
		Else
		...
		oDanfe:Say(171, nAuxH + 2, "V.ST", oFont08N:oFont)
		//**************************************************************************|
		nAuxH += aTamCol[12]
	...
	If _cAliqAtu <> _cAliqAnt

		oDanfe:Box(nLinha-12,000,nLinha,603)	                             
		
		nLinha := nLinha-5
		
		oDanfe:Say(nLinha, nAuxH+2,"SubTotal Aliq. ICMS:"+_cAliqAnt, oFontAli:oFont) // ALIQ
		
		nAuxH := _nAuxH-17
		
		nAuxH2 := len(aAux[1][9][nY]) + (nAuxH + (aTamCol[9]) - RetTamTex(aAux[1][9][nY], oFont08:oFont, oDanfe))
		oDanfe:Say(nLinha, nAuxH2, Transform(_nValTot,"@E 999,999,999.99"), oFontAli:oFont) // V. TOTAL
		nAuxH += aTamCol[9]
	
		nAuxH2 := len(aAux[1][10][nY]) + (nAuxH + (aTamCol[10]) - RetTamTex(aAux[1][10][nY], oFont08:oFont, oDanfe))
		oDanfe:Say(nLinha, nAuxH2, Transform(_nBasIcm,"@E 999,999,999.99"), oFontAli:oFont) // BC. ICMS
		nAuxH += aTamCol[10]
			
		nAuxH2 := len(aAux[1][11][nY]) + (nAuxH + (aTamCol[11]) - RetTamTex(aAux[1][11][nY], oFont08:oFont, oDanfe))
		oDanfe:Say(nLinha, nAuxH2-2, Transform(_nValIcm,"@E 999,999,999.99"), oFontAli:oFont) // V. ICMS
		nAuxH += aTamCol[11]
	//CAIO************************************************************************************************************************|				
		nAuxH2 := len(aAux[1][12][nY]) + (nAuxH + (aTamCol[12]) - RetTamTex(aAux[1][12][nY], oFont08:oFont, oDanfe))    //|  
		oDanfe:Say(nLinha, nAuxH2-2, Transform(_nValSt,"@E 999,999,999.99"), oFontAli:oFont) // V.ST                                              //|
		nAuxH += aTamCol[12]                                                                                            //|
	//************************************************************************************************************************|
			
		nAuxH2 := len(aAux[1][13][nY]) + (nAuxH + (aTamCol[13]) - RetTamTex(aAux[1][13][nY], oFont08:oFont, oDanfe))    
		oDanfe:Say(nLinha, nAuxH2, Transform(_nValIpi,"@E 999,999,999.99"), oFontAli:oFont) // V.IPI                                             
		nAuxH += aTamCol[13]                                                                                            
	...
	If !Empty(aAux[1][3][nY]) .And. aAux[1][3][nY] <> "-" 
		...
		_nValTot  += RetVal(aAux[1][9][nY])
		_nBasIcm  += RetVal(aAux[1][10][nY])
		_nValIcm  += RetVal(aAux[1][11][nY])
		_nValST   += RetVal(aAux[1][12][nY])
		_nValIpi  += RetVal(aAux[1][13][nY])
...
//CAIO************************************************************************************************************************|				
		nAuxH2 := len(aAux[1][12][nY]) + (nAuxH + (aTamCol[12]) - RetTamTex(aAux[1][12][nY], oFont08:oFont, oDanfe))    //|  
		oDanfe:Say(nLinha, nAuxH2, aAux[1][12][nY], oFont08:oFont) // V.ST                                              //|
		nAuxH += aTamCol[12]                                                                                            //|
//************************************************************************************************************************|
		
		nAuxH2 := len(aAux[1][13][nY]) + (nAuxH + (aTamCol[13]) - RetTamTex(aAux[1][13][nY], oFont08:oFont, oDanfe))    
		oDanfe:Say(nLinha, nAuxH2, aAux[1][13][nY], oFont08:oFont) // V.IPI                                             
		nAuxH += aTamCol[13]                                                                                            

		
		nAuxH2 := len(aAux[1][14][nY]) + (nAuxH + (aTamCol[14]) - RetTamTex(aAux[1][14][nY], oFont08:oFont, oDanfe)) 
		oDanfe:Say(nLinha, nAuxH2, aAux[1][14][nY], oFont08:oFont) // A.ICMS
		nAuxH += aTamCol[14]
		
		nAuxH2 := len(aAux[1][15][nY]) + (nAuxH + (aTamCol[15]) - RetTamTex(aAux[1][15][nY], oFont08:oFont, oDanfe)) 
		oDanfe:Say(nLinha, nAuxH2, aAux[1][15][nY], oFont08:oFont) // A.IPI
	EndIf
	...
	If nY == nLenItens
			...
			nAuxH2 := len(aAux[1][11][nY]) + (nAuxH + (aTamCol[11]) - RetTamTex(aAux[1][11][nY], oFont08:oFont, oDanfe))
			oDanfe:Say(nLinha, nAuxH2+4, Transform(_nValIcm,"@E 999,999,999.99"), oFontAli:oFont) // V. ICMS
			nAuxH += aTamCol[11]
		//CAIO************************************************************************************************************************|				
			nAuxH2 := len(aAux[1][12][nY]) + (nAuxH + (aTamCol[12]) - RetTamTex(aAux[1][12][nY], oFont08:oFont, oDanfe))    //|  
			oDanfe:Say(nLinha, nAuxH2+6, Transform(_nValSt,"@E 999,999,999.99"), oFontAli:oFont) // V.ST                                              //|
			nAuxH += aTamCol[12]                                                                                            //|
		//************************************************************************************************************************|
				
			nAuxH2 := len(aAux[1][13][nY]) + (nAuxH + (aTamCol[13]) - RetTamTex(aAux[1][13][nY], oFont08:oFont, oDanfe))    
			oDanfe:Say(nLinha, nAuxH2+4, Transform(_nValIpi,"@E 999,999,999.99"), oFontAli:oFont) // V.IPI                                             
			nAuxH += aTamCol[13]                                                                                            
...
If nfolha==1
	If lMv_Logod
//		oDanfe:SayBitmap(042,000,cLogoD,095,096)
		oDanfe:SayBitmap(042,000,cLogoD,085,045)
	Else
		oDanfe:SayBitmap(042,000,cLogo,095,096)
	EndIF
Endif
...
For nZ := 1 To nLenDet
    ...
    lPontilhado := .T.	//caio maior garcia
...
Static Function fNewIten(_aNewAux,_aBase)
...
For _nx := 1 To Len(_aNewAux[1][14]) 

	AADD(_aOrdTmp,{_aNewAux[1,14,_nx],_nx,_nItem})
...
ASORT(_aOrdTmp,,,{|x,y| x[1]>y[1]})

For _nx := 1 To Len(_aOrdTmp)

	If _aOrdTmp[_nx,1] <> 0

		AADD(_aOrdem,{_aOrdTmp[_nx,1],_aOrdTmp[_nx,2],_aOrdTmp[_nx,3]})
	
		For _ny := 1 To Len(_aOrdBkp)
		
			If _aOrdTmp[_nx,3] == _aOrdBkp[_ny,3] .And. _aOrdBkp[_ny,1] <> _aOrdtMP[_nX,1]                                  
			
					AADD(_aOrdem,{_aOrdBkp[_ny,1],_aOrdBkp[_ny,2],_aOrdbkp[_ny,3]})
...
For _nx := 1 To Len(_aNewAux[1])
 
	For _ny := 1 To Len(_aOrdem)
	
		_aNewAux[1,_nx,_ny] := _aBase[1,_nx,_aOrdem[_ny,2]]
...
Static Function RetVal(_cVal)

Local _nVal    := 0
Local _nTam    := Len(_cVal)
Local _cValTmp := ""
Local _nx      := 0           
Local _cSubs   := ""
...
For _nx := 1 To _nTam
	...
	_cSubs := Subs(_cVal,_nx,1)
       
	If _cSubs <> "." .And. _cSubs <> ","
	
		_cValTmp := _cValTmp+AllTrim(_cSubs)
