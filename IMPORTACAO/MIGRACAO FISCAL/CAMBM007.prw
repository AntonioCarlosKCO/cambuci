...
Rotina para importação de Movimentos Internos - SD3.
...
Private cCadastro	:= "Importação Movimentações Internas - Estoque (SD3)"
Private aCabec		:= {}
Private aLinhas		:= {}
Private cNomArq		:= ""
Private cPathArq 	:= ""
...
AADD(aSays, OemToAnsi("Esta função tem o objetivo de importar as Movimentações Internas informadas no arquivo para o sistema"))
...
If nOpca == 1
	If MSGYESNO("Confirma a importação dos arquivos do diretório selecionado ?", cCadastro)
		...
		oProcess := MsNewProcess():New({ |lFim| CAMBM07I(@lFim)}, "Importação das Movimentações Internas (Estoque)", "Processando...", .T. )
		oProcess:Activate()		
	EndIf
EndIf
...
Rotina para importação das Movimentações Internas.
...
If aParam[1] == "1" //Gera Log
	FWMsgRun(, {|| CAMBM07R(aCabec, aLinLog)}, "Gerando o Log da Importação... Aguarde...") 
EndIf
				...
				While (nErrLin <= nLinhas)
			    	...
			    	cBuffer := ALLTRIM(MEMOLINE(cErroTemp,, nErrLin))
			     	If (UPPER(SUBSTR(cBuffer, LEN(cBuffer)-7, LEN(cBuffer))) == "INVALIDO") 
						cValInc := "ERRO GRAVAÇÃO SD3 = "+ALLTRIM(cBuffer)
						Exit									
			    	EndIf 
				EndDo
...
oReport  := TReport():New("CAMBM007", "LOG DA IMPORTAÇÃO - MOVIMENTAÇÃO INTERNA - ESTOQUE", "", {|oReport| PrintReport(oReport, aDadosLog)}, "Log da Importação do arquivo para a tabela SD3 (Movimentos Internos)")
...
Do Case
	...
	Case cTipoLog == "5"
		cDescRet := "Falha na gravação da Movimentação Interna (SD3)."
	Case cTipoLog == "6"
